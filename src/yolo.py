...
class YoloLossBlock(nn.Cell):
    ...
    def construct(self, prediction, pred_xy, pred_wh, y_true, gt_box, input_shape, feature_scale):
        """
        ...
        # loss = c_iou_loss_me * self.loss_iou + confidence_loss * self.loss_confi + class_loss * self.loss_cls
        loss = c_iou_loss_me * 4 + confidence_loss * 0.2 + class_loss
        self.scalar_summary("c_iou_loss_me", c_iou_loss_me)
        self.scalar_summary("confidence_loss", confidence_loss)
        self.scalar_summary("class_loss", class_loss)
...
class YoloWithLossCell(nn.Cell):
    ...
    def construct(self, x, y_true_0, y_true_1, y_true_2, gt_0, gt_1, gt_2, input_shape):
        ...
        loss_l = self.loss_big(*yolo_out[0], y_true_0, gt_0, input_shape, 'l')
        loss_m = self.loss_me(*yolo_out[1], y_true_1, gt_1, input_shape, 'm')
        loss_s = self.loss_small(*yolo_out[2], y_true_2, gt_2, input_shape, 's')

        self.scalar_summary("loss_l", loss_l)
        self.scalar_summary("loss_m", loss_m)
        self.scalar_summary("loss_s", loss_s)
        return loss_l * 0.8 + loss_m + loss_s * 2.0
...
class CIOU(nn.Cell):
    ...
    def construct(self, boxes1, boxes2):
        ...
        boxes1_x1y1x2y2 = self.concat((boxes1_x1, boxes1_y1, boxes1_x2, boxes1_y2))
        ...
        boxes2_x1y1x2y2 = self.concat((boxes2_x1, boxes2_y1, boxes2_x2, boxes2_y2))

        boxes1_x1y1x2y2 = self.reshape(boxes1_x1y1x2y2, (-1, 4))
        boxes2_x1y1x2y2 = self.reshape(boxes2_x1y1x2y2, (-1, 4))

        box_p_area = (boxes1_x1y1x2y2[..., 2:3] - boxes1_x1y1x2y2[..., 0:1]) * (
                    boxes1_x1y1x2y2[..., 3:4] - boxes1_x1y1x2y2[..., 1:2])
        box_gt_area = (boxes2_x1y1x2y2[..., 2:3] - boxes2_x1y1x2y2[..., 0:1]) * (
                    boxes2_x1y1x2y2[..., 3:4] - boxes2_x1y1x2y2[..., 1:2])
        x_1 = self.max(boxes1_x1y1x2y2[..., 0:1], boxes2_x1y1x2y2[..., 0:1])
        x_2 = self.min(boxes1_x1y1x2y2[..., 2:3], boxes2_x1y1x2y2[..., 2:3])
        y_1 = self.max(boxes1_x1y1x2y2[..., 1:2], boxes2_x1y1x2y2[..., 1:2])
        y_2 = self.min(boxes1_x1y1x2y2[..., 3:4], boxes2_x1y1x2y2[..., 3:4])
        intersection = (y_2 - y_1) * (x_2 - x_1)
        xc_1 = self.min(boxes1_x1y1x2y2[..., 0:1], boxes2_x1y1x2y2[..., 0:1])
        xc_2 = self.max(boxes1_x1y1x2y2[..., 2:3], boxes2_x1y1x2y2[..., 2:3])
        yc_1 = self.min(boxes1_x1y1x2y2[..., 1:2], boxes2_x1y1x2y2[..., 1:2])
        yc_2 = self.max(boxes1_x1y1x2y2[..., 3:4], boxes2_x1y1x2y2[..., 3:4])
        ...
        # enclose_c2 = self.pow(xc_2 - xc_1,2) + self.pow(yc_2 - yc_1,2)

        enclose_c2 = self.pow(xc_2 - xc_1, 2) + self.pow(yc_2 - yc_1, 2)
        enclose_c2 = enclose_c2 + self.eps
        ...
        h = y_2 - yc_1 + self.eps
        w_gt = xc_2 - x_1
        h_gt = yc_2 - y_1 + self.eps
        # p2 = self.pow(boxes1[..., 0:1]-boxes2[..., 0:1],2) + self.pow(boxes1[..., 1:2]-boxes2[..., 1:2],2)

        # p2 = (boxes1[..., 0:1]-boxes2[..., 0:1])*(boxes1[..., 0:1]-boxes2[..., 0:1]) + (boxes1[..., 1:2]-boxes2[..., 1:2])*(boxes1[..., 1:2]-boxes2[..., 1:2])
        p2 = self.pow(x2 - x1, 2) + self.pow(y2 - y1, 2)

        atan1 = self.atan(self.div(self.cast(w_gt, ms.float32), self.cast(h_gt, ms.float32)))

        atan2 = self.atan(self.div(self.cast(w, ms.float32), self.cast(h, ms.float32)))

        v = self.mul(self.div(self.pow(atan1 - atan2, 2), self.pow(self.cast(np.pi, ms.float32), 2)), 4)

        a = self.div(v, self.cast(1 - iou + v + self.eps, ms.float32))
        # ciou = iou - 1.0 * p2 / enclose_c2 - 1.0 * a * v
        ciou = iou - self.div(self.cast(p2, ms.float32), self.cast(enclose_c2, ms.float32)) - self.mul(a, v)
        # ciou = iou - self.div(self.cast(p2, ms.float32), self.cast(enclose_c2, ms.float32))
        # ciou = iou
        ciou = C.clip_by_value(ciou, -1.0, 1.0)
